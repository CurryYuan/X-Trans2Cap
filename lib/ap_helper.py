...
def parse_predictions_pg(end_points, config_dict):
    """ Parse predictions to OBB parameters and suppress overlapping boxes
    ...
    pred_corners_3d_upright_camera = end_points['bbox_corner'].cpu().numpy()
    pred_sem_cls = end_points['sem_cls']
    bsize, K = pred_corners_3d_upright_camera.shape[:2]  # K==num_proposal
    nonempty_box_mask = np.ones((bsize, K))
    ...
    pred_mask = nonempty_box_mask * end_points['bbox_mask'].cpu().numpy()
    batch_pred_map_cls = []  # a list (len: batch_size) of list (len: num of predictions per sample) of tuples of pred_cls, pred_box and conf (0-1)
    for i in range(bsize):
        batch_pred_map_cls.append([(pred_sem_cls[i, j].item(), pred_corners_3d_upright_camera[i, j], 1) \
                                   for j in range(K) if pred_mask[i, j] == 1])
    end_points['batch_pred_map_cls'] = batch_pred_map_cls
