...
class ProposalModule(nn.Module):
    ...
    def forward(self, xyz, features, data_dict):
        ...
        data_dict['aggregated_vote_features'] = features.permute(0, 2,
                                                                 1).contiguous()  # (batch_size, num_proposal, 128)
        data_dict[
            'aggregated_vote_inds'] = sample_inds  # (batch_size, num_proposal,) # should be 0,1,2,...,num_proposal

        # --------- PROPOSAL GENERATION ---------
        net = self.proposal(features)
        data_dict = self.decode_scores(net, data_dict, self.num_class, self.num_heading_bin, self.num_size_cluster,
                                       self.mean_size_arr)
    ...
    def decode_pred_box(self, data_dict):
        ...
        for i in range(batch_size):
            ...
            pred_obbs.append(torch.from_numpy(pred_obb_batch).cuda().unsqueeze(0))

        pred_bboxes = torch.cat(pred_bboxes, dim=0)  # batch_size, num_proposals, 8, 3
        pred_obbs = torch.cat(pred_obbs, dim=0).float()  # (batch_size, num_proposals, 7)

        data_dict["bbox_corner"] = pred_bboxes
        data_dict['bbox_obb'] = pred_obbs
    ...
    def decode_scores(self, net, data_dict, num_class, num_heading_bin, num_size_cluster, mean_size_arr):
        ...
        data_dict['heading_residuals_normalized'] = heading_residuals_normalized
        # B x num_proposal x num_heading_bin
        data_dict['heading_residuals'] = heading_residuals_normalized * (np.pi / num_heading_bin)
        ...
        # processed box info
        self.decode_pred_box(data_dict)  # bounding box corner coordinates
